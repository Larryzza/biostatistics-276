...
2) $Q(\beta_{t},\beta_{t+1})=\delta N(\beta_{t}, c*\tilde\sum_{t}) + (1-\delta) N(\beta_{t}, \sum_{0})$ in which $\delta\sim Binomial(p_\delta)$. here we set $p_\delta=0.8$, and set $\sum_{0}$ same as the prior $\sum$ of $\beta$.
...
simulation begin (set eta=1, a=3, b=3)
...
# simulation
px.da.mh.mcmc.sim <- px.da.mh.sim(nsim=nsim, burn=burn)
