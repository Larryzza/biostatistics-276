...
.cal_ll <- function(beta){
  ...
  out <- log(out) %>% sum - 1/2 * beta %*% bvar_prior %*% t(beta) 
}

mh.sim <- function(nsim=1000, burn=0, delta=0.8, seed=1996){
  set.seed(seed)
  ...
  bvar <- diag(num_beta)*10^(-10)
  
  for(i in 1:nsim.total){
    if(i <= 3000){
      bvar <- (bvar * (i - 1) + t(beta) %*% beta + diag(num_beta)*10^(-10))/i
    }
    beta_temp_1 <- rmvnorm(n=1, mean = beta, sigma = bvar)
    beta_temp_2 <- rmvnorm(n=1, mean = beta, sigma = bvar_prior)
    beta_temp <- beta_temp_1 * delta_set[nsim.total] + 
      beta_temp_2 * (1-delta_set[nsim.total])
    P0 <- .cal_ll(beta)
    P1 <- .cal_ll(beta_temp)
    ratio <- P1 - P0
...
# simulation
mcmc.sim <- mh.sim(nsim=nsim, burn=burn, delta=0.7)
# plot
par(mfrow=c(2,3))
for(i in 1:5){
  plot(1:nsim, mcmc.sim$beta[,i], type="l")
}
par(mfrow=c(2,3))
for(i in 1:5){
  acf(mcmc.sim$beta[,i])
...
results <- apply(mcmc.sim$beta, 2, 
                 function(x){quantile(x, c(0.025, 0.5, 0.975))}) %>% round(.,2)
...
# simulation
mcmc.sim <- da.mh.sim(nsim=nsim, burn=burn)
# plot
par(mfrow=c(2,3))
for(i in 1:5){
  plot(1:nsim, mcmc.sim$beta[,i], type="l")
...
&= \exp(-\frac{1}{2}(\beta^{'}(X^{'}X+\sum/\alpha^2)\beta-2*\beta X^{'}w/\alpha))
\end{aligned}$$

This is a normal kernel. Then we found that $\beta|z,\tau\sim N(\frac{X^{'}w}{\alpha X^{'}X+\sum/\alpha},\frac{1}{X^{'}X+\sum/\alpha^2})$.
...
&=\frac{1}{\sqrt{2\pi\alpha^2}}\exp(-\frac{(w-\alpha X\beta)^2}{2\alpha^2})*(\alpha^2)^{-a-1}\exp(-b/\alpha^2)\\
&\propto (\alpha^2)^{-a-3/2}\exp(-(\frac{(w-\alpha X\beta)^2}{2}+b)/\alpha^2)
\end{aligned}$$

This is a gamma kernel. Then we know that $\alpha^2\sim Gamma(\frac{2a+1}{2},\frac{2b+(w-\alpha X\beta)^2}{2})$

```{r}
px.da.mh.sim <- function(nsim=1000, burn=0,
                         a=3, b=3, seed=1996,
                         X=as.matrix(df[,-1]), 
                         Y=as.matrix(df[,1])){
  set.seed(seed)
  ...
  for(i in 1:nsim.total){
    ...
    bvar <- solve(t(X)%*%X+bvar_prior/alpha.sq)
    beta <- rmvnorm(n=1, mean = bmean, sigma = bvar)
    
    #alpha.sq
    alpha.sq <- rinvgamma(n = 1, shape = (2*a+1)/2, 
                          scale = (2*b+t(w-sqrt(alpha.sq)*X%*%t(beta))%*%
                                     (w-sqrt(alpha.sq)*X%*%t(beta)))/2)
      
    if(i > burn.num){ 
      i1 <- i - burn.num
      beta.ch[i1,] = beta
      z.ch[i1,] = z
      alpha.sq.ch[i1] = alpha.sq
    }
    print(c(alpha.sq, (2*b+t(w-X%*%t(beta))%*%(w-X%*%t(beta)))/2))
  }
  return(list(beta=beta.ch))
}
```
...
# simulation
mcmc.sim <- px.da.mh.sim(nsim=nsim, burn=burn)
# plot
par(mfrow=c(2,3))
for(i in 1:5){
  plot(1:nsim, mcmc.sim$beta[,i], type = "l")
