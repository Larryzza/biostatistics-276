...
# simulation
mcmc.sim <- mh.sim(nsim=nsim, burn=burn, delta=0.7, c=1)
# plot
par(mfrow=c(2,3))
for(i in 1:5){
...
par(mfrow=c(2,3))
for(i in 1:5){
  acf(mcmc.sim$beta[,i],main = substitute(beta[x], list(x = i)))
}
...
This is difficult to make it a kernel. Then we consider use MH method to simulate $\alpha^2$

```{r}
.alpha.sq.ll <- function(alpha.sq,beta,w,a,b,X){
  out <- log(alpha.sq^(-a-2/3))-
    ((2*b+t(w-sqrt(alpha.sq)*X%*%t(beta))%*%
        (w-sqrt(alpha.sq)*X%*%t(beta)))/2)/alpha.sq
  return(out)
}
px.da.mh.sim <- function(nsim=1000, burn=0, eta=1,
                         a=3, b=3, seed=1996,
                         X=as.matrix(df[,-1]), 
                         Y=as.matrix(df[,1])){
  set.seed(seed)
  ...
  for(i in 1:nsim.total){
    ...
    #alpha.sq
    alpha.sq_temp <- rnorm(n = 1, mean = alpha.sq, sd = eta)
    P0 <- .alpha.sq.ll(alpha.sq,beta,w,a,b,X)
    P1 <- .alpha.sq.ll(alpha.sq_temp,beta,w,a,b,X)
    ratio <- P1 - P0
    if(log(runif(1)) < ratio){
      alpha.sq <- alpha.sq_temp
    }
