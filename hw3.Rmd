...
hmc.sim.gibbs <- function(initial_value, nsim=1000, burn=0.5, seed=199609){
  ...
  for(i in 1:nsim.total){
    ...
    d0 <- lapply(input_var, function(x)x/10)
...
```{r}
nsim <- 3000; m <- c(rep(100,5),5,10); epsilon <- 0.3; nn <- 30
...
par(mfrow=c(2,3))
for(i in 1:5){
  acf(df[,i], main = paste0(names(df)[i], "(Gibbs)"))
}
...
par(mfrow=c(2,3))
for(i in 1:5){
  acf(df_hmc[,i], main = paste0(names(df_hmc)[i], "(HMC)"))
}
```
